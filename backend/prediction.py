...
def predict_diabetes(data: schemas.DiabetesInput) -> Dict[str, Any]:
    ...
    try:
        # --- Clinical Override (Strict Prediction) ---
        # Lab values are high risk indicators regardless of model.
        # Fasting Glucose >= 126 or HbA1c >= 6.5 is clinical for Diabetes.
        if (data.glucose and data.glucose >= 126) or (data.HbA1c_level and data.HbA1c_level >= 6.5):
             return {
                 "prediction": "High Risk",
                 "raw": 1,
                 "note": "Clinical lab value override (Glucose/HbA1c)"
             }
        ...
        # We also have HbA1c and Glucose now, but the current ML model (BRFSS) 
        # doesn't use them. If we want to use them in ML, we'd need a different model.
...
def explain_diabetes(data: schemas.DiabetesInput):
    ...
    feature_names = ['Hypertension', 'HighChol', 'BMI', 'Smoker', 'HeartDisease', 'PhysActivity', 'GenHlth', 'Gender', 'AgeBucket']
